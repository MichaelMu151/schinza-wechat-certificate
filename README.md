...
> (version **1.10.1**). Upstream `main` and GitHub Releases **do not** include
...
If `data/capture_debug.log` has no new lines, WeChat 4.x is not being intercepted:
its Chromium helper (`WeChatAppEx`) ignores the macOS HTTP proxy. This build also
starts mitmproxy **local capture** for WeChat. Allow the network-filter prompt if
shown, then fully quit and reopen WeChat.
...
On macOS, **无人值守归档** (v1.10.1) finishes one account before the next:
...
`app/safari_handoff.py`, currently 958,640; remesaure after a display
change with `python -m app.safari_handoff --mouse`, or set
`SCHINZA_GO_BUTTON`). Cards are not deleted. Rate-limit errors stop the
