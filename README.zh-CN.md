...
- 主按钮是 **拉取并归档正文**，没有文章卡片、全选或批量勾选；
- 进度只显示篇数、页数、用时和正文成功/失败；
- 点击一次后会自动翻页，列表完成后自动拉取正文；
...
5. **历史文章** → 选择公众号（默认全部历史）→ **拉取并归档正文**。程序会自动翻页并拉取全部正文，无需勾选。
...
一天，在凭证管理中续约后，对同一公众号再点一次 **拉取并归档正文**。目录固定为
`data/archives/公众号名/`，已完成正文会自动跳过。不要提高并发或连续重试。
